...
public class NfcRW extends Activity {
    ...
    EditText etSector, etBlock, etData , etKeya;
    ...
    private byte[] b3;
    byte[] code = {(byte)0xFF,(byte)0xFF,(byte)0xFF,(byte)0xFF,(byte)0xFF,(byte)0xFF};
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Log.d( "demo" , toHexString (code ) );
        byte[]  demo = toByteArray ("1234567890ABCDEF" );
        Log.d( "demo" , toHexString ( demo )  );
    ...
    public void writeTag(Tag tag, String str) {
        ...
        code = toByteArray (etKeya.getText().toString());
        ...
        try {
            ...
            int wSector = Integer.parseInt(etSector.getText().toString());
            wSector = wSector % 16;
            int wBlock = Integer.parseInt(etBlock.getText().toString());
            wBlock = wSector * 4 + wBlock % 3;


            str = str + "\n";
            CodeAuth = mfc.authenticateSectorWithKeyA(wSector, code);

            if (CodeAuth == false) {
                Toast.makeText(mContext, "密碼錯誤", 1).show();
                return;
            }

            b0 = new byte[16];
            System.arraycopy(b1, 0, b0, 0, 16);
            mfc.writeBlock(wBlock, b0);
            str += ByteArrayChange.ByteArrayToHexString(b0);
            System.out.println("Block" + wBlock + ": " + str);
            mfc.close();

            if (CodeAuth == true) {
                Toast.makeText(mContext, "写入成功", 1).show();
                return;
            }
        ...
        /*
           ...
           // byte[] b1 = toByteArray(str);
           // Log.d( "demo" , toHexString ( b1 )  );
            if (b1.length <= 720) {
                ...
                if (!(b1.length % 16 == 0)) {
                    ...
                    mfc.close();
                    Toast.makeText(this, "写入成功A", Toast.LENGTH_SHORT).show();
                    return;
                } else {
                    str = str + "\n";
                    for (int i = 1, j = 1; i <= num; i++) {
                    ...
                    mfc.close();
                    Toast.makeText(this, "写入成功B", Toast.LENGTH_SHORT).show();
                    return;
                }
            } else {
    ...
    /*
    版权声明：本文为CSDN博主「Lic_LiveTime」的原创文章，遵循 CC 4.0 BY-SA 版权协议，转载请附上原文出处链接及本声明。
    原文链接：https://blog.csdn.net/Lic_LiveTime/java/article/details/53003241
    */
